...
function AdminPage() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(`${base_url}api/users/users_data/`, data);
  ...
  const deleteHandling = (detail) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        //   id:
        // }
        axios.delete(`${base_url}api/users/delete_user/${detail.id}/`).then((response) => {
  ...
  return (
    <>
            ...
            <div className="relative flex h-16 items-center justify-between">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                <div className="relative ml-3  flex">
                  <button className="Btn1 ml-2" onClick={()=> dispatch(userLogout())}>
                    <div className="sign1">
              ...
              {usersData ? (
                ...
                >
                  <td className=" items-center">
                    <div className="ml-6 w-[50px] h-[50px] bg-cover bg-center bg-no-repeat rounded-lg" style={(user.profile) ? {backgroundImage: `url(${base_url}${user.profile})`}:{backgroundImage: `url(./avatar.jpg)`}}></div>
                  </td>
                  <th
                    scope="row"
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center">{user.phone_number}</div>
                  </td>
                  <td className="px-6 py-4">{(user.is_listed)?(<><div className="flex items-center"><div className="h-[8px] w-[8px] bg-green-800 rounded-lg"></div><div className="ml-1">Active</div></div></>):(<><div className="flex items-center"><div className="h-[8px] w-[8px] bg-red-800 rounded-lg"></div><div className="ml-1">Non-Active</div></div></>)}</td>
                  <td className="px-6 py-4">
                    <button
                    ...
                    <button className="bg-red-700 mt-1 hover:bg-red-600 text-white font-semibold py-2 px-4 border border-gray-400 rounded shadow ml-1" onClick={()=>deleteHandling({id:user.id})}>
                      Delete
                    </button>
                  </td>
                </tr>))
      ...
      >
        ...
        <EditModal action={modalSub} userdata={modalPassData} setModalIsOpenToFalse={setModalIsOpenToFalse} showemail={true} setUsersData={setUsersData}/>
      </Modal>
    </>
  );
}
